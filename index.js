    ...
    educators = {
        ...
        'quotes' : {
            'true' : function () {
                ...
                var self = this,
                    value = self.data.originalValue || self.toString(),
                    next, nextNext, prev, nextValue, PUNCTUATION, WORD,
                    WHITE_SPACE;
                ...
                PUNCTUATION = self.PUNCTUATION_NODE;
                WORD = self.WORD_NODE;
                WHITE_SPACE = self.WHITE_SPACE_NODE;
                next = self.next;
    ...
    function attachFactory(events) {
        return function (retext) {
            ...
            for (event in events) {
                ...
                iterator = -1;
                while (method = methods[++iterator]) {
                    PunctuationNode.on(event, method);
                }
            }
        };
    }
